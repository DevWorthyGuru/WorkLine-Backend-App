...
export class PersonalWeekMonthYearPage {
  ...
  constructor(private _router: Router, private _route: ActivatedRoute, private utilService: UtilService, private timeService: TimeService) {
    ...
    this.getEmployeeWeekDataForPersonal({
      start_date: this.date.start_date,
      end_date: this.date.end_date
    });
  }

  async getEmployeeWeekDataForPersonal(month: any) {
    await this.utilService.showLoading();
    const self = this;
    this.timeService.getEmployeeWeekDataForPersonal(month).then(async function (o) {
      ...
      if (o.success) {
        ...
        if (self.taskList.length > 0) {
          self.getEmployeeWeekTimeDataForPersonal({
  ...
  async getEmployeeWeekTimeDataForPersonal(month: any) {
    await this.utilService.showLoading();
    const self = this;
    this.timeService.getEmployeeWeekTimeDataForPersonal(month).then(async function (o) {
      await self.utilService.closeLoading();
      if (o.success) {
  ...
  onSelectTask($event): void {
    this.getEmployeeWeekTimeDataForPersonal({
      start_date: this.date.start_date,
      end_date: this.date.end_date,
      mode: 1,
      id: $event.id
